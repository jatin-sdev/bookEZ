...
function CheckoutContent() {
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-8 pt-24">
        {/* Header */}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="flex gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h4 className="font-bold text-sm line-clamp-1">{order.eventName || 'Event Name'}</h4>
                            <p className="text-xs text-slate-500 mt-1">{order.eventLocation || 'Venue Location'}</p>
                            <p className="text-xs text-slate-500">{new Date(Number(order.eventDate || Date.now())).toLocaleDateString()}</p>
                        </div>
                    </div>
